...
const metrics = [
  { value: 52, suffix: '%', label: 'redução de custos', color: 'text-primary' },
  { value: 38, suffix: '%', label: 'Aumento na conversão', color: 'text-accent' },
  { value: 0, suffix: '', label: 'Leads perdidos', color: 'text-foreground' },
];
  ...
  return (
    <section ref={ref} id="resultados" className="py-24 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        {/* Metrics Grid - now with neon cards */}
        <div className="grid md:grid-cols-3 gap-8 max-w-4xl mx-auto mb-20">
          {metrics.map((metric, index) => (
            <motion.div
              ...
              className="text-center neon-card p-8 rounded-2xl holo-shimmer"
            >
              ...
              {/* Accent bar */}
              <div className={`mt-4 mx-auto h-0.5 w-12 rounded-full ${index === 0 ? 'bg-primary' : index === 1 ? 'bg-accent' : 'bg-foreground/30'}`} />
            </motion.div>
          ))}
        </div>
        ...
        <motion.div
          ...
          className="neon-card p-8 md:p-12 rounded-3xl max-w-4xl mx-auto holo-shimmer"
        >
          <div className="flex flex-col md:flex-row items-center gap-8">
            <div className="flex-shrink-0">
              <div className="w-20 h-20 rounded-full bg-gradient-to-br from-primary to-accent flex items-center justify-center text-3xl font-bold text-primary-foreground">T</div>
            </div>
            <div className="text-center md:text-left">
              ...
              <div>
                ...
                <p className="text-muted-foreground text-sm">Fundador da Orvia</p>
              </div>
            </div>
          </div>
        </motion.div>
